...
const VALID_TRANSITIONS: Record<TaskState, TaskState[]> = {
  ...
  FAILED:         ["PENDING"],
  CANCELED:       [],
  REVISION_REQUESTED: ["WORKING", "CANCELED"],
};
...
export class RunManager {
  ...
  getRun(_team: string, runId?: string): { found: true; run: TeamRun } | { found: false } {
    if (runId) {
      ...
      // Try loading from disk — the run may have been created by a
      // different RunManager instance (gateway re-activation race).
      const diskRun = this.tryLoadRunFromDisk(runId);
      if (diskRun) return { found: true, run: diskRun };
    ...
    // Last resort: check disk for runs not yet in memory
    const diskRun = this.tryLoadAnyWorkingRunFromDisk();
    if (diskRun) return { found: true, run: diskRun };
  ...
  /**
   * Try loading a specific run from disk into memory.
   * Used when the run exists on disk but not in this RunManager's activeRuns
   * (e.g., it was created by a different RunManager instance).
   */
  private tryLoadRunFromDisk(runId: string): TeamRun | null {
    try {
      const filePath = path.join(this.baseDir, "active", `${runId}.json`);
      const data = fsSync.readFileSync(filePath, "utf-8");
      const run = JSON.parse(data) as TeamRun;
      this.activeRuns.set(run.id, run);
      return run;
    } catch {
      return null;
  ...
  /**
   * Scan active directory for any WORKING run not yet in memory.
   */
  private tryLoadAnyWorkingRunFromDisk(): TeamRun | null {
    try {
      const activeDir = path.join(this.baseDir, "active");
      const files = fsSync.readdirSync(activeDir).filter((f) => f.endsWith(".json"));
      for (const file of files) {
        const data = JSON.parse(fsSync.readFileSync(path.join(activeDir, file), "utf-8")) as TeamRun;
        if (data.status === "WORKING" && data.id && !this.activeRuns.has(data.id)) {
          this.activeRuns.set(data.id, data);
          return data;
        }
      }
    } catch {
  ...
  /**
   * Increment the round_count for a run (used by fail-loopback enforcement).
   */
  incrementRoundCount(_team: string, runId?: string): number {
    const run = this.resolveRun(runId);
    if (!run) return 0;
    run.round_count = (run.round_count ?? 0) + 1;
    run.updated_at = Date.now();
    return run.round_count;
  }
  ...
  updateTask(
    _team: string,
    taskId: string,
    updates: Partial<Pick<TeamTask, "status" | "result" | "message" | "assigned_to" | "routing_reason" | "deliverables" | "learning" | "workflow_stage" | "revision_count" | "revision_feedback">>,
  ): TeamTask | undefined {
    ...
    if (updates.revision_count !== undefined) task.revision_count = updates.revision_count;
    if (updates.revision_feedback !== undefined) task.revision_feedback = updates.revision_feedback;
