...
export function teamSendTool(ctx: ToolContext) {
  return {
    ...
    label: "Team Send",
    description:
      "Send messages to team members or publish events. Requires 'team' parameter for non-team agents. Use 'to' for direct messages, 'topic' for event publishing.",
    ...
    async execute(
      ...
      if (params.to) {
        ...
        // Notify recipient(s) with a heartbeat so they process the message
        const callerRunId = resolveRunIdFromSession(ctx.sessionKey);
        const recipientNames = params.to === "all"
          ? members.filter((m) => m !== teamCtx.member)
          : [params.to];
