...
export function parseConfig(raw: unknown): AgentTeamsConfig {
  ...
  for (const [name, teamRaw] of Object.entries(teams)) {
    ...
    result.teams[name] = {
      ...
      routing: {
        fallback: (prop(routing, "fallback") as string | undefined) ?? "orchestrator",
      },
      workflow: {
        max_rounds: (prop(workflow, "max_rounds") as number | undefined) ?? 10,
        timeout: (prop(workflow, "timeout") as number | undefined) ?? 600,
      ...
      knowledge: {
        ...
        consolidation_timeout: (prop(knowledge, "consolidation_timeout") as number | undefined) ?? 30,
