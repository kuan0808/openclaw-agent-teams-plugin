...
export async function askAgent(
  ...
  // Strip ANSI escape codes (some CLI renderers emit them even with CI=true)
  ...
  // The CLI may prefix output with non-JSON lines; find the JSON object
  const jsonStart = cleaned.indexOf("{");
  if (jsonStart === -1) {
    throw new Error(`No JSON found in openclaw CLI output: ${cleaned.slice(0, 200)}`);
  }
